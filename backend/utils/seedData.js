...
const seedData = async () => {
  try {
    ...
    if (userCount === 0) {
      const hr = new User({ name: 'HR Manager', email: 'hr@example.com', password: 'password123', role: 'hr' });
      const candidate = new User({ name: 'John Doe', email: 'candidate@example.com', password: 'password123', role: 'candidate' });
      await hr.save();
